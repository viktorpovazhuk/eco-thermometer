...
int LoRaClass::endPacket(bool async)
{
    ...
    // don't use async mode
    //  if ((async) && (_onTxDone))
    //    writeRegister(REG_DIO_MAPPING_1, 0x40); // DIO0 => TXDONE

    // check version -> works correctly:
    // even when irq is incorrect
      ...
      if (!async)
      {
        // TODO: didn't setup irq flags?
        // Bohdan plugged in dio0?
        // NEED TORESET SPI! MB, previous spi connection is still active ->
        // can't receive correct data
